...
async function beesideFinalizarCompra() {
  ...
  // Registro do pedido no Supabase é "melhor esforço": se as tabelas
  // orders/order_items ainda não existirem ou o insert falhar por
  // qualquer motivo, a compra continua sendo uma demonstração visual
  // (ver aviso "Compra visual preparada para apresentação do projeto"
  // no carrinho.html) — não bloqueia a animação de sucesso.
  ...
  if (erroPedido) {
    console.warn(
      "Não foi possível registrar o pedido no Supabase (seguindo como compra visual):",
      erroPedido.message,
    );
